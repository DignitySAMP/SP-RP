...
new const g_MaxVehicleColors = sizeof g_VehicleColors;

CMD:carcolorlist(playerid, params[]) 
{
    ...
    ShowPlayerDialog(playerid, 1337, DIALOG_STYLE_MSGBOX, "Color List", color_list, "Close", "");
    return 1;
}

CMD:spraytagcolors(playerid, params[]) 
{
    ...
    format(color_list, sizeof ( color_list ), "%s\n\nWhen formatting your graffiti text, use {A3A3A3}[[{{{}]]{DEDEDE} (i.e. [[123]]) to use colors.\n\
        You can also use {A3A3A3}[[n]]{DEDEDE} to force a new line.", color_list ) ;
